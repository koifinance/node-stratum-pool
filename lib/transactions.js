...
var generateOutputTransactions = function (poolRecipient, recipients, rpcData, poolOptions) {
    ...
    if (rpcData.dev_fund) {
        if (rpcData.dev_fund.dev_1 && rpcData.dev_fund.dev_2 && rpcData.dev_fund.amount_1 && rpcData.dev_fund.amount_2) { 
            var payeeScript = util.addressToP2sh(rpcData.dev_fund.dev_1);
            ...
            var payeeScript = util.addressToP2sh(rpcData.dev_fund.dev_2);
            txOutputBuffers.push(Buffer.concat([
    ...
    if (rpcData.ghostnode_payments_started == true) {
        if (rpcData.ghostnode.payee && rpcData.ghostnode.amount) { 
            var payeeScript = util.addressToP2sh(rpcData.ghostnode.payee);
            txOutputBuffers.push(Buffer.concat([
